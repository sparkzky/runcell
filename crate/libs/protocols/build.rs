...
impl ProtobufCustomizeCallback for GenSerde {
    ...
    fn field(&self, field: &FieldDescriptor) -> ProtobufCustomize {
        if field.proto().type_() == Type::TYPE_ENUM {
            ProtobufCustomize::default().before(
                "#[cfg_attr(feature = \"with-serde\", serde(serialize_with = \
                 \"crate::serialize_enum_or_unknown\", deserialize_with = \
                 \"crate::deserialize_enum_or_unknown\"))]",
            )
        } else if field.proto().type_() == Type::TYPE_MESSAGE && field.is_singular() {
            ProtobufCustomize::default().before(
                "#[cfg_attr(feature = \"with-serde\", serde(serialize_with = \
                 \"crate::serialize_message_field\", deserialize_with = \
                 \"crate::deserialize_message_field\"))]",
            )
        } else {
...
fn real_main() -> Result<(), std::io::Error> {
    ...
    // so there is a struct named 'Box', we should replace Box<Self> to
    // ::std::boxed::Box<Self> to avoid the conflict.
    replace_text_in_file(
        "src/oci.rs",
        "self: Box<Self>",
