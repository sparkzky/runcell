...
// TODO(burgerdev): remove condition here and below after upgrading to oci_spec
// > 0.7.
#[cfg(target_os = "linux")]
...
impl From<grpc::Linux> for oci::Linux {
    fn from(from: grpc::Linux) -> Self {
        ...
        if !from.MaskedPaths().is_empty() {
            ...
            // masked_paths is MUST be set None as it's initialized with default paths which
            // are useless for kata.
            oci_linux.set_masked_paths(None);
        }
        if !from.ReadonlyPaths().is_empty() {
            oci_linux.set_readonly_paths(Some(from.ReadonlyPaths().to_vec()));
        } else {
            // readonly_paths is MUST be set None as it's initialized by default paths which
            // are useless for kata.
