...
/// A safe version of [`PathBuf`] pinned to an underlying filesystem object to
/// protect from `TOCTTOU` style of attacks.
///
/// A [`PinnedPathBuf`] is a resolved path buffer pinned to an underlying
/// filesystem object, which guarantees:
/// - the value of [`PinnedPathBuf::as_path()`] never changes.
/// - the path returned by [`PinnedPathBuf::as_path()`] is always a symlink.
/// - the filesystem object referenced by the symlink
///   [`PinnedPathBuf::as_path()`] never changes.
/// - the value of [`PinnedPathBuf::target()`] never changes.
///
/// Note:
/// - Though the filesystem object referenced by the symlink
///   [`PinnedPathBuf::as_path()`] never changes, the value of
///   `fs::read_link(PinnedPathBuf::as_path())` may change due to filesystem
///   operations.
/// - The value of [`PinnedPathBuf::target()`] is a cached version of
///   `fs::read_link(PinnedPathBuf::as_path())` generated when creating the
///   `PinnedPathBuf` object.
/// - It's a sign of possible attacks if `[PinnedPathBuf::target()]` doesn't
///   match `fs::read_link(PinnedPathBuf::as_path())`.
/// - Once the [`PinnedPathBuf`] object gets dropped, the [`Path`] returned by
///   [`PinnedPathBuf::as_path()`] becomes invalid.
///
/// With normal [`PathBuf`], there's a race window for attackers between time to
/// validate a path and time to use the path. An attacker may maliciously change
/// filesystem object referenced by the path by using symlinks to compose an
/// attack.
///
/// The [`PinnedPathBuf`] is introduced to protect from such attacks, by using
/// the `/proc/self/fd/xxx` files on Linux. The `/proc/self/fd/xxx` file on
/// Linux is a symlink to the real target corresponding to the process's file
/// descriptor `xxx`. And the target filesystem object referenced by the symlink
/// will be kept stable until the file descriptor has been closed. Combined with
/// `O_PATH`, a safe version of `PathBuf` could be built by:
/// - Generate a safe path from `root` and `path` by using
///   [`crate::scoped_join()`].
/// - Open the safe path with O_PATH | O_CLOEXEC flags, say the fd number is
///   `fd_num`.
/// - Read the symlink target of `/proc/self/fd/fd_num`.
/// - Compare the symlink target with the safe path, it's safe if these two
///   paths equal.
...
impl PinnedPathBuf {
    ...
    ///
    /// The `path` must be a subdirectory of `root`, otherwise error will be
    /// returned.
    ...
    ///
    /// The target path is cached version of
    /// `fs::read_link(PinnedPathBuf::as_path())` generated when creating
    /// the `PinnedPathBuf` object. On the other hand, the value of
    /// `fs::read_link(PinnedPathBuf::as_path())` may change due to underlying
    /// filesystem operations. So it's a sign of possible attacks if
    /// `PinnedPathBuf::target()` does not match
    ...
    /// Open a direct child of the filesystem objected referenced by the
    /// `PinnedPathBuf` object.
...
mod tests {
    ...
    fn test_pinned_path_buf() {
        ...
        // Remove the target file and validate that we could still read data from the
        // pinned path.
