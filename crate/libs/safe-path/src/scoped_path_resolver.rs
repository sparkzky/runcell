...
/// Resolve `unsafe_path` to a relative path, rooted at and constrained by
/// `root`.
///
/// The `scoped_resolve()` function assumes `root` exists and is an absolute
/// path. It processes each path component in `unsafe_path` as below:
...
/// - recursively resolve to the real path if it's a symlink. All symlink
///   resolutions will be constrained by `root`.
...
/// components in the returned PathBuf are not modified (in other words are not
/// replaced with symlinks on the filesystem) after this function has returned.
/// You may use [crate::PinnedPathBuf] to protect from such TOCTOU attacks.
...
/// Safely join `unsafe_path` to `root`, and ensure `unsafe_path` is scoped
/// under `root`.
///
/// The `scoped_join()` function assumes `root` exists and is an absolute path.
/// It safely joins the two given paths and ensures:
/// - The returned path is guaranteed to be scoped inside `root`.
/// - Any symbolic links in the path are evaluated with the given `root` treated
///   as the root of the filesystem, similar to a chroot.
///
...
/// - `unsafe_path`: the path to evaluated and joint with `root`. It is unsafe
///   since it may try to escape from the `root` by using "../" or symlinks.
///
/// # Security
/// On success return, the `scoped_join()` function guarantees that:
/// - The resulting PathBuf must be a child path of `root` and will not contain
///   any symlink path components (they will all get expanded).
/// - When expanding symlinks, all symlink path components must be resolved
///   relative to the provided `root`. In particular, this can be considered a
///   userspace implementation of how chroot(2) operates on file paths.
/// - Non-existent path components are unaffected.
///
/// Note that the guarantees provided by this function only apply if the path
/// components in the returned string are not modified (in other words are not
/// replaced with symlinks on the filesystem) after this function has returned.
/// You may use [crate::PinnedPathBuf] to protect from such TOCTTOU attacks.
...
#[cfg(test)]
mod tests {
    use std::{fs::DirBuilder, os::unix::fs};
