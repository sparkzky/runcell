...
use crate::{PinnedPathBuf, scoped_join, scoped_resolve};
...
///
/// The `ScopedDirBuilder` is a counterpart for `DirBuilder`, with safety
/// enhancements of:
/// - ensuring the new directories are created under a specified `root`
///   directory.
/// - ensuring all created directories are still scoped under `root` even under
///   symlink based attacks.
/// - returning a [PinnedPathBuf] for the last level of directory, so it could
///   be used for other operations safely.
#[derive(Debug)]
...
impl ScopedDirBuilder {
    /// Create a new instance of `ScopedDirBuilder` with with default
    ...
    /// parent directories.
    ///
    /// Parents that do not exist are created with the same security and
    ...
    /// builder.
    ///
    /// This is a helper to create subdirectory with an absolute path, without
    /// stripping off `self.root`. So error will be returned if path does
    /// start with `self.root`. It is considered an error if the directory
    /// already exists unless recursive mode is enabled.
