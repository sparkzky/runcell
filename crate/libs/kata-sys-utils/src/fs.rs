...
/// Reflink copy src to dst, and falls back to regular copy if reflink copy
/// fails.
///
/// # Safety
/// The `reflink_copy()` doesn't preserve permission/security context for the
/// copied file, so caller needs to take care of it.
pub fn reflink_copy<S: AsRef<Path>, D: AsRef<Path>>(src: S, dst: D) -> Result<()> {
    ...
    // Make sure dst's parent exist. If dst is a regular file, then unlink it for
    // later copy.
    if dst_path.exists() {
        if !dst_path.is_file() {
...
fn do_reflink_copy(src: File, dst: File) -> Result<()> {
    ...
    // pub unsafe fn ficlone(fd: libc::c_int, data: libc::c_ulang) ->
    // Result<libc::c_int>
    ioctl_write_int!(ficlone, FS_IOC_MAGIC, FS_IOC_FICLONE);

    // Safe because the `src` and `dst` are valid file objects and we have checked
    // the result.
