...
//!
//! These utilities and helpers are specially designed and implemented to
//! support container runtimes on Linux systems, so they may not be generic
//! enough.
//!
//! # Quotation from [mount(2)](https://man7.org/linux/man-pages/man2/mount.2.html)
//!
//! A call to mount() performs one of a number of general types of operation,
//! depending on the bits specified in mountflags. The choice of which operation
//! to perform is determined by testing the bits set in mountflags, with the
//! tests being conducted in the order listed here:
//! - Remount an existing mount: mountflags includes MS_REMOUNT.
//! - Create a bind mount: mountflags includes MS_BIND.
//! - Change the propagation type of an existing mount: mountflags includes one
//!   of MS_SHARED, MS_PRIVATE, MS_SLAVE, or MS_UNBINDABLE.
...
//! only the per-mount-point flags. This is particularly useful for setting or
//! clearing the "read-only" flag on a mount without changing the underlying
//! filesystem. Specifying mountflags as:            MS_REMOUNT | MS_BIND |
//! MS_RDONLY will make access through this mountpoint read-only, without
//! affecting other mounts.
//!
//! # Safety
//!
//! Mount related operations are sensitive to security flaws, especially when
//! dealing with symlinks. There are several CVEs related to file path handling,
//! for example [CVE-2021-30465](https://github.com/opencontainers/runc/security/advisories/GHSA-c3xm-pvg7-gh7r).
//!
//! So some design rules are adopted here:
//! - all mount variants (`bind_remount_read_only()`, `bind_mount()`,
//!   `Mounter::mount()`) assume that all received paths are safe.
//! - the caller must ensure safe version of `PathBuf` are passed to mount
//!   variants.
//! - `create_mount_destination()` may be used to generated safe `PathBuf` for
//!   mount destinations.
//! - the `safe_path` crate should be used to generate safe `PathBuf` for
//!   general cases.
...
use nix::{
    NixPath,
    mount::{MntFlags, MsFlags, mount},
    unistd,
};
use oci_spec::runtime as oci;

use crate::{fs::is_symlink, sl};
...
/// Get the device and file system type of a mount point by parsing
/// `/proc/mounts`.
...
///
/// For a normal mount, the destination will always be a directory. For bind
/// mount, the destination must be a directory if the source is a directory,
/// otherwise the destination must be a normal file. If directories are created,
/// their permissions are initialized to MountPerm.
///
/// # Safety
///
/// Every container has a root filesystems `rootfs`. When creating bind mounts
/// for a container, the destination should always be within the container's
/// `rootfs`. Otherwise it's a serious security flaw for container to
/// read/override host side filesystem contents. Please refer to following CVEs
/// for example:
/// - [CVE-2021-30465](https://github.com/opencontainers/runc/security/advisories/GHSA-c3xm-pvg7-gh7r)
///
/// To ensure security, the `create_mount_destination()` function takes an extra
/// parameter `root`, which is used to ensure that `dst` is within the specified
/// directory. And a safe version of `PathBuf` is returned to avoid TOCTTOU type
/// of flaws.
pub fn create_mount_destination<S: AsRef<Path>, D: AsRef<Path>, R: AsRef<Path>>(
    ...
    if fs_type == "bind" {
        // The source and destination for bind mounting must be the same type: file or
        // directory.
...
///
/// # Safety
/// Caller needs to ensure safety of the `dst` to avoid possible file path based
/// attacks.
...
/// Bind mount `src` to `dst` with a custom propagation type, optionally in
/// readonly mode if `readonly` is true.
///
/// Propagation type: MsFlags::MS_SHARED or MsFlags::MS_SLAVE
/// MsFlags::MS_SHARED is used to bind mount the sandbox path to enable `exec`
/// (in case of FC jailer).
...
impl Mounter for kata_types::mount::Mount {
    ...
    fn mount<P: AsRef<Path>>(&self, target: P) -> Result<()> {
        ...
        // - In the case of remounting with changed data (data != ""), need to call
        //   mount
...
/// Take fstab style mount options and parses them for use with a standard
/// mount() syscall.
...
fn parse_mount_flags(mut flags: MsFlags, flag_str: &str) -> Option<MsFlags> {
    ...
    // - group: Allow an ordinary user to mount the filesystem if one of that user’s
    ...
    //   in /run/mount on systems without a regular mtab) so that this same user can
    ...
    //   noexec, nosuid, and nodev (unless overridden by subsequent options, as in
...
// Do mount, optionally change current working directory if `chdir` is not
// empty.
...
/// When the size of mount options is bigger than one page, try to reduce the
/// size by compressing the `lowerdir` option for overlayfs. The assumption is
/// that lower directories for overlayfs often have a common prefix.
...
///
/// # Safety
/// Caller needs to ensure safety of the `path` to avoid possible file path
/// based attacks.
            ...
            match e.kind() {
                ...
                io::ErrorKind::WouldBlock => {
                    ...
                    if time_now.duration_since(start_time).as_millis() > timeout as u128 {
                        warn!(
                            sl!(),
                            "failed to umount {} in {} ms because of EBUSY, try again with lazy \
                             umount",
                            path.display(),
...
///
/// If `mountpoint` is empty or doesn't exist, `umount_all()` is a noop.
/// Otherwise it will try to unmount all filesystems mounted at `mountpoint`
...
pub fn umount_all<P: AsRef<Path>>(mountpoint: P, lazy_umount: bool) -> Result<()> {
    ...
    loop {
        if let Err(e) = umount2(mountpoint.as_ref(), lazy_umount) {
            ...
            // done. It can also indicate a few other things (such as invalid flags) which
            // we unfortunately end up squelching here too.
