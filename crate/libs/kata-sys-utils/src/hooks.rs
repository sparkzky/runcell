...
use crate::{eother, sl, validate::valid_env};

const DEFAULT_HOOK_TIMEOUT_SEC: i32 = 10;

/// A simple wrapper over `oci::Hook` to provide `Hash, Eq`.
///
/// The `oci::Hook` is auto-generated from protobuf source file, which doesn't
/// implement `Hash, Eq`.
...
impl HookStates {
    ...
    /// states:
    /// - path (string, REQUIRED) with similar semantics to IEEE Std 1003.1-2008
    ///   execv's path. This specification extends the IEEE standard in that
    ...
    /// - timeout (int, OPTIONAL) is the number of seconds before aborting the
    ///   hook. If set, timeout MUST be greater than zero.
    ///
    /// The OCI spec also defines the context to invoke hooks, caller needs to
    /// take the responsibility to setup execution context, such as
    /// namespace etc.
    pub fn execute_hook(
        &mut self,
        hook: &oci::Hook,
    ...
    /// It will first be called before creating the VMM when creating the
    /// sandbox, so hooks could be used to setup environment for the VMM,
    /// such as creating tap device etc. It will also be called during
    /// starting containers, to setup environment for those containers.
    ///
    /// The execution result will be recorded for each hook. Once a hook returns
    /// success, it will not be invoked anymore.
