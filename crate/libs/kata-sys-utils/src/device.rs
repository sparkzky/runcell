...
/// Get major and minor number of the device or of the device hosting the
...
pub fn get_devid_for_blkio_cgroup<P: AsRef<Path>>(path: P) -> Result<Option<(u64, u64)>> {
    ...
    if md.is_dir() || md.is_file() {
        ...
        // Note that we need to get the major/minor of the block device instead of
        // partition, e.g. /dev/sda instead of /dev/sda3, because blkio cgroup
        // works with block major/minor.
        let id = md.dev();
...
/// Get the block device major/minor number from a partition/block
/// device(itself).
///
/// For example, given the dev_t of /dev/sda3 returns major and minor of
/// /dev/sda. We rely on the fact that if /sys/dev/block/$major:$minor/partition
/// exists, then it's a partition, and find its parent for the real device.
fn get_block_device_id(dev: stat::dev_t) -> Result<Option<(u64, u64)>> {
    ...
    // If 'partition' file exists, then it's a partition of the real device, take
    // its parent. Otherwise it's already the real device.
    loop {
...
mod tests {
    ...
    fn test_get_devid() {
        // let (major, minor) =
        // get_devid_for_blkio_cgroup("/dev/vda1").unwrap().unwrap();
