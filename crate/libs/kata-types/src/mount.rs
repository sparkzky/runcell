...
use crate::{build_path, handler::HandlerManager};
...
/// KATA_EPHEMERAL_VOLUME_TYPE creates a tmpfs backed volume for sharing files
/// between containers.
...
/// KATA_MOUNT_INFO_FILE_NAME is used for the file that holds direct-volume
/// mount info
...
/// DEFAULT_KATA_DIRECT_VOLUME_ROOT_PATH is the default root path used for
/// concatenating with the direct-volume mount info file path
...
/// In CoCo scenario, we support force_guest_pull to enforce container image
/// guest pull without remote snapshotter.
...
/// Get the root path used for concatenating with the direct-volume mount info
/// file path.
...
pub struct Mount {
    ...
    /// or a dummy. Path values for bind mounts are either absolute or
    /// relative to the bundle. A mount is a bind mount if it has either
    /// bind or rbind in the options.
    pub source: String,
    /// Destination of mount point: path inside container. This value MUST be an
    /// absolute path.
    ...
    /// Intermediate path to mount the source on host side and then passthrough
    /// to vm by shared fs.
...
/// to consume a host block device and mount it as a filesystem inside the guest
/// VM.
...
/// Represents a Kata virtual volume, encapsulating information for extra mount
/// options and direct volumes.
///
/// Direct communication channels between components like snapshotters,
/// `kata-runtime`, `kata-agent`, `image-rs`, and CSI drivers are often
/// expensive to build and maintain.
///
/// Therefore, `KataVirtualVolume` is introduced as a common infrastructure to
/// encapsulate additional mount options and direct volume information. It
/// serves as a superset of `NydusExtraOptions` and `DirectVolumeMountInfo`.
///
/// The interpretation of other fields within this structure is determined by
/// the `volume_type` field.
///
/// # Volume Types:
///
/// - `KATA_VIRTUAL_VOLUME_IGNORE`: All other fields should be ignored/unused.
///
/// - `KATA_VIRTUAL_VOLUME_DIRECT_BLOCK`:
///   - `source`: The directly assigned block device path.
///   - `fs_type`: Filesystem type.
///   - `options`: Mount options.
///   - `direct_volume`: Additional metadata to pass to the agent regarding this
///     volume.
///
/// - `KATA_VIRTUAL_VOLUME_IMAGE_RAW_BLOCK` or
...
///
/// - `KATA_VIRTUAL_VOLUME_IMAGE_NYDUS_BLOCK` or
///   `KATA_VIRTUAL_VOLUME_LAYER_NYDUS_BLOCK`:
///   - `source`: Path to nydus meta blob.
///   - `fs_type`: Filesystem type.
///   - `nydus_image`: Configuration information for nydus image.
///   - `dm_verity`: Disk `dm-verity` information.
///
/// - `KATA_VIRTUAL_VOLUME_IMAGE_NYDUS_FS` or
///   `KATA_VIRTUAL_VOLUME_LAYER_NYDUS_FS`:
///   - `source`: Path to nydus meta blob.
///   - `fs_type`: Filesystem type.
///   - `nydus_image`: Configuration information for nydus image.
///
...
impl KataVirtualVolume {
    ...
    /// Serializes the virtual volume object to a JSON string and encodes the
    /// string with base64.
    ...
    /// Decodes and deserializes a virtual volume object from a base64 encoded
    /// JSON string.
    ...
    /// Decode and deserialize a virtual volume object from base64 encoded json
...
///
/// The `volume_path` is base64-url-encoded and then safely joined to the
/// `prefix`. `safe_path` is OS-specific.
...
///
/// The `bindmount` format is typically `/path/to/dir` or
/// `/path/to/dir:ro[:rw]`. This function extracts the real path (without the
/// suffix ":ro" or ":rw") and the mode.
...
///
/// This function disregards any provided `rootfs_mounts`. Instead, it
/// forcefully creates a single, default `KataVirtualVolume` specifically for
/// guest-pull operations. This volume's representation is then base64-encoded
/// and added as the only option to a new, singular `Mount` entry, which becomes
/// the sole item in the returned `Vec<Mount>`. This ensures that when guest
/// pull is active, the root filesystem is exclusively configured via this
/// virtual volume.
...
mod tests {
    ...
    fn test_get_nydus_extra_options_v5() {
        ...
        assert_eq!(
            extra_option.source,
            "/var/lib/containerd/io.containerd.snapshotter.v1.nydus/snapshots/9/fs/image/image.\
             boot"
        );
        assert_eq!(
    ...
    fn test_get_nydus_extra_options_v6() {
        ...
        assert_eq!(
            extra_option.source,
            "/var/lib/containerd/io.containerd.snapshotter.v1.nydus/snapshots/201/fs/image/image.\
             boot"
        );
        assert_eq!(
            extra_option.snapshot_dir,
    ...
    fn test_adjust_rootfs_mounts_basic_success() {
        ...
        // 2. Verify Mount's fields and ensure source, destination, typ with default
        //    value
