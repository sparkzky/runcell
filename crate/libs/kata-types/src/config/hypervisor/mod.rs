...
//!
//! The configuration information for hypervisors is complex, and different
//! hypervisors require different configuration information. To make it flexible
//! and extensible, we build a multi-layer architecture to manipulate hypervisor
//! configuration information.
//!
//! - **Vendor layer**: The `HypervisorVendor` structure provides hook points
//!   for vendors to customize the configuration for its deployment.
//! - **Hypervisor plugin layer**: Provides hook points for different
//!   hypervisors to manipulate the configuration information.
//! - **Hypervisor common layer**: Handles generic logic for all types of
//!   hypervisors.
//!
//! These three layers are applied in order. Changes made by the vendor layer
//! will be visible to the hypervisor plugin layer and the common layer. Changes
//! made by the plugin layer will only be visible to the common layer.
//!
//! Ideally the hypervisor configuration information should be split into
//! hypervisor specific part and common part. But the Kata 2.0 has adopted a
//! policy to build a superset for all hypervisors, so let's contain it...
...
use super::{ConfigOps, ConfigPlugin, TomlConfig, default};
use crate::{
    annotations::KATA_ANNO_CFG_HYPERVISOR_PREFIX, eother, resolve_path, sl, validate_path,
};
...
mod qemu;
pub use self::qemu::{HYPERVISOR_NAME_QEMU, QemuConfig};

mod ch;
pub use self::ch::{CloudHypervisorConfig, HYPERVISOR_NAME_CH};

mod remote;
pub use self::remote::{HYPERVISOR_NAME_REMOTE, RemoteConfig};

mod rate_limiter;
pub use self::rate_limiter::{DEFAULT_RATE_LIMITER_REFILL_TIME, RateLimiterConfig};
...
pub struct BlockDeviceInfo {
    ...
    ///
    /// In case of a storage driver like devicemapper where a container's root
    /// file system is backed by a block device, the block device is passed
    /// directly to the hypervisor for performance reasons. This flag
    /// prevents the block device from being passed to the hypervisor,
    /// shared fs is used instead to pass the rootfs.
    #[serde(default)]
    pub disable_block_device_use: bool,

    /// Block storage driver to be used for the hypervisor in case the container
    /// rootfs is backed by a block device. Options include:
    /// - `virtio-scsi`
    /// - `virtio-blk`
    /// - `nvdimm`
    #[serde(default)]
    ...
    ///
    ///   - io_uring Linux io_uring API. This provides the fastest I/O
    ///     operations on Linux, requires kernel > 5.1 and qemu >= 5.0.
    #[serde(default)]
    ...
    ///
    /// It is the memory address space for the NVDIMM device. If set block
    /// storage driver (`block_device_driver`) to `nvdimm`, should set
    /// `memory_offset` to the size of block device.
    #[serde(default)]
    pub memory_offset: u64,

    /// Enable vhost-user storage device, default false.
    ///
    /// Enabling this will result in some Linux reserved block type major range
    /// 240-254 being chosen to represent vhost-user devices.
    #[serde(default)]
    pub enable_vhost_user_store: bool,

    /// The base directory specifically used for vhost-user devices.
    ///
    /// Its sub-path `block` is used for block devices; `block/sockets` is where
...
impl BlockDeviceInfo {
    /// Adjust the configuration information after loading from configuration
    /// file.
...
pub struct BootInfo {
    ...
    /// Block storage driver to be used for the VM rootfs when backed by a block
...
impl BootInfo {
    ...
    ///
    /// New parameters are added before the original to let the original ones
    /// take priority.
...
pub struct CpuInfo {
    ...
    /// - Unspecified or `0`: Set to actual number of physical cores or maximum
    ///   vCPUs supported by KVM if exceeded
    /// - `> 0` and `<= number of physical cores`: Set to specified number
    /// - `> number of physical cores`: Set to actual number of physical cores
    ///   or maximum vCPUs supported by KVM if exceeded
    ///
    /// # WARNING
    ///
...
pub struct DebugInfo {
    ...
    /// # WARNING
    ///
    /// Dumping guest memory can be time-consuming and use significant disk
    /// space.
    #[serde(default)]
...
impl TopologyConfigInfo {
    ...
    pub fn new(toml_config: &TomlConfig) -> Option<Self> {
        // Firecracker does not support PCIe Devices, so we should not initialize such a
        // PCIe topology for it. If the case of fc hit, just return None.
...
pub struct MachineInfo {
    ...
    ///
    /// Example: `machine_accelerators =
    /// "nosmm,nosmbus,nosata,nopit,static-prt,nofw"`
    #[serde(default)]
...
impl MemoryInfo {
    /// Adjusts the configuration information after loading from a configuration
    /// file.
    ///
...
pub struct NetworkInfo {
    ...
    /// Sets the maximum inbound bandwidth for network I/O in bits/sec for the
    /// sandbox/VM.
    ///
    /// In QEMU, this is implemented using classful `qdiscs HTB` (Hierarchy
    /// Token Bucket) to manage traffic. A value of `0` indicates an
    /// unlimited rate (default).
    #[serde(default)]
    pub rx_rate_limiter_max_rate: u64,

    /// Sets the maximum outbound bandwidth for network I/O in bits/sec for the
    /// sandbox/VM.
    ///
    /// In QEMU, this is implemented using classful `qdiscs HTB` (Hierarchy
    /// Token Bucket) and `ifb` (Intermediate Functional Block) to manage
    /// traffic. A value of `0` indicates an unlimited rate (default).
    #[serde(default)]
...
impl NetworkInfo {
    /// Adjusts the network configuration information after loading from a
    /// configuration file. (Currently, this method performs no
    /// adjustments.)
    pub fn adjust_config(&mut self) -> Result<()> {
        Ok(())
    }
    ...
    /// (Currently, this method performs no specific validations beyond basic
    /// type checks.)
    pub fn validate(&self) -> Result<()> {
        Ok(())
    }
...
pub struct SecurityInfo {
    ...
    /// It contains the UID, GID, supplementary groups, and the user name of the
    /// non-root user that will be used to run the VMM process.
    #[serde(default)]
    ...
    ///
    /// Toggling this setting may activate different hardware features, ranging
    /// from memory encryption to both memory and CPU-state encryption and
    /// integrity. The Kata Containers runtime dynamically detects the
    /// available feature set and aims to enable the largest possible one.
    #[serde(default)]
    ...
    ///
    /// Hooks are executed in the runtime namespace of the guest. See the
    /// official Open Containers Initiative (OCI) documentation for more
    /// details: <https://github.com/opencontainers/runtime-spec/blob/v1.0.1/config.md#posix-platform-hooks>
    ///
    /// Warnings will be logged if any error is encountered while scanning for
    /// hooks, but it will not abort container execution.
    #[serde(default)]
    pub guest_hook_path: String,

    /// Initdata provides dynamic configuration (such as policies, configs, and
    /// identity files) in an encoded format that users inject into the TEE
    /// Guest upon CVM launch.
    ///
    ...
    ///
    /// Each member of the list is a regular expression, representing the base
    /// name of the annotation (e.g., "path" for
    /// "io.katacontainers.config.hypervisor.path").
    #[serde(default)]
    pub enable_annotations: Vec<String>,

    /// Defines the Intel Quote Generation Service (QGS) port exposed from the
    /// host.
    #[serde(
    ...
    /// comma-separated list of seccomp sandbox features to control the syscall
    /// access. For example, `seccompsandbox=
    /// "on,obsolete=deny,spawn=deny,resourcecontrol=deny"`
    /// Note: "elevateprivileges=deny" doesn't work with daemonize option, so
    /// it's removed from the seccomp sandbox Another note: enabling this
    /// feature may reduce performance, you may enable /proc/sys/net/core/bpf_jit_enable to reduce the impact. see https://man7.org/linux/man-pages/man8/bpfc.8.html
...
impl SecurityInfo {
    /// Adjusts the security configuration information after loading from a
    /// configuration file.
    ///
    ...
    /// Checks whether a given annotation key is enabled based on the
    /// `enable_annotations` list.
    ///
    /// Returns `true` if the annotation key (after removing the
    /// `KATA_ANNO_CFG_HYPERVISOR_PREFIX`) matches any of the regular
    /// expressions in `enable_annotations`.
...
/// Configuration information for shared filesystems, such as virtio-9p and
/// virtio-fs.
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
pub struct SharedFsInfo {
    ...
    ///
    /// If not set, the default is an empty list, meaning all annotations are
    /// rejected.
    #[serde(default)]
    ...
    ///
    /// Format example: `["-o", "arg1=xxx,arg2", "-o", "hello world",
    /// "--arg3=yyy"]`
    ///
    ...
    /// - `never`: Metadata, data, and pathname lookups are not cached in the
    ///   guest. They are always fetched from the host, and any changes are
    ///   immediately pushed to the host.
    /// - `auto`: Metadata and pathname lookup cache expires after a configured
    ///   amount of time (default is 1 second). Data is cached while the file is
    ///   open (close-to-open consistency).
    /// - `always`: Metadata, data, and pathname lookups are cached in the guest
    ///   and never expire.
    #[serde(default)]
    ...
    /// This is the `msize` used for 9p shares. It represents the number of
    /// bytes used for the 9p packet payload.
    #[serde(default)]
    pub msize_9p: u32,
}

impl SharedFsInfo {
    /// Adjusts the shared filesystem configuration after loading from a
    /// configuration file.
    ///
    /// Handles default values for `shared_fs` type, `virtio-fs` specific
    /// settings (daemon path, cache mode, DAX), and `virtio-9p` msize.
    pub fn adjust_config(&mut self) -> Result<()> {
    ...
    pub fn validate(&self) -> Result<()> {
        match self.shared_fs.as_deref() {
            ...
            Some(VIRTIO_9P) => {
                ...
                {
                    return Err(eother!(
                        "Invalid 9p configuration msize 0x{:x}, min value is 0x{:x}, max value is \
                         0x{:x}",
                        self.msize_9p,
                        default::MIN_SHARED_9PFS_SIZE_MB,
                        default::MAX_SHARED_9PFS_SIZE_MB
                    ));
                }
                Ok(())
            }
    ...
    /// default `virtio_fs_cache` mode, and `virtio_fs_is_dax` with
    ...
    ///
    /// Checks the validity of the `virtio_fs_daemon` path (unless in inline
    /// mode), `virtio_fs_cache` mode, and `virtio_fs_is_dax` with
    /// `virtio_fs_cache_size`.
...
pub struct RemoteInfo {
    ...
    /// GPU specific annotations (currently only applicable for Remote
    /// Hypervisor). Specifies the number of GPUs required for the Kata VM.
    #[serde(default)]
...
pub struct Factory {
    ...
    /// When enabled, new VMs may be created from a template to speed up
    /// creation.
...
pub struct Hypervisor {
    ...
    #[serde(default)]
    pub ctlpath: String,
    /// List of valid annotation values for the hypervisor control executable
    /// path.
    ///
    /// Each member of the list is a path pattern as described by `glob(3)`.
    /// The default is an empty list (all annotations rejected) if not set.
    ...
    /// Disables the runtime customizations applied when running on top of a
    /// VMM.
    ///
    /// Setting this to `true` will make the runtime behave as it would when
    ...
    /// A sandbox annotation used to specify the host path to the
    /// `prefetch_files.list` for the container image being used. The
    /// runtime will pass this path to the Hypervisor to search for the
    /// corresponding prefetch list file.
    ///
...
impl Hypervisor {
    /// Validates the path of the hypervisor executable against configured
    /// patterns.
    pub fn validate_hypervisor_path<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        validate_path_pattern(&self.valid_hypervisor_paths, path)
    }

    /// Validates the path of the hypervisor control executable against
    /// configured patterns.
...
impl ConfigOps for Hypervisor {
    /// Adjusts the overall hypervisor configuration after loading from the
    /// configuration file.
    ///
    /// This method iterates through configured hypervisors, calls their
    /// respective plugin adjustments, and then recursively adjusts nested
    /// configuration structs like `blockdev_info`, `boot_info`, etc. It
    /// also resolves paths for `prefetch_list_path`.
    ...
    ///
    /// This method iterates through configured hypervisors, calls their
    /// respective plugin validations, and then recursively validates nested
    /// configuration structs and various paths (`path`, `ctlpath`,
    /// `jailer_path`, `prefetch_list_path`).
