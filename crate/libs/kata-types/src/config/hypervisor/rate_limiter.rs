...
impl RateLimiterConfig {
    /// Helper function: Creates a `TokenBucketConfig` based on the provided
    /// rate and burst. Returns `None` if the `rate` is 0.
    ...
    ///
    /// If both `band_rate` and `ops_rate` are 0 (indicating no rate limiting
    /// configured), it returns `None`. Otherwise, it returns
    /// `Some(RateLimiterConfig)` containing the configured options.
    pub fn new(
        band_rate: u64,
        ops_rate: u64,
        ...
        // Use pattern matching to concisely handle the final
        // `Option<RateLimiterConfig>` return. If both bandwidth and ops are
        // `None`, the entire config is `None`. Otherwise, return `Some` with
        // the actual configured options.
        match (bandwidth, ops) {
            (None, None) => None,
