...
pub struct Agent {
    /// If enabled, the agent will log additional debug messages to the system
    /// log.
    ...
    /// process a CreateContainerRequest. It's also used to ensure that
    /// workloads, especially those involving large image pulls within the
    /// guest, have sufficient time to complete.
    #[serde(
    ...
    /// The following example can be used to load two kernel modules with
    /// parameters:
    ///  - kernel_modules=["e1000e InterruptThrottleRate=3000,3000,3000 EEE=1",
    ///    "i915 enable_ppgtt=0"] The first word is considered as the module
    ///    name and the rest as its parameters. Container will not be started
    ///    when:
    /// - A kernel module is specified and the modprobe command is not installed
    ///   in the guest or it fails loading the module.
    /// - The module is not available in the guest or it doesn't met the guest
    ///   kernel requirements, like architecture and version.
    #[serde(default)]
