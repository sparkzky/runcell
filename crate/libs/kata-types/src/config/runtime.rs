...
use super::default;
use crate::{
    config::{ConfigOps, TomlConfig},
    eother,
    mount::split_bind_mounts,
    validate_path,
};
...
pub struct Runtime {
    /// Runtime name: Plan to support virt-container, linux-container,
    /// wasm-container
    #[serde(default)]
    pub name: String,
    ...
    /// If enabled, the runtime will log additional debug messages to the system
    /// log.
    ...
    /// they may break compatibility, and are prepared for a big version
    /// bump.
    #[serde(default)]
    pub experimental: Vec<String>,

    /// Determines how the VM should be connected to the container network
    /// interface.
    ///
    /// Options:
    /// - macvtap: used when the Container network interface can be bridged
    ///   using macvtap.
    /// - none: used when customize network. Only creates a tap device. No veth
    ///   pair.
    /// - tcfilter: uses tc filter rules to redirect traffic from the network
    ///   interface provided by plugin to a tap interface connected to the VM.
    #[serde(default)]
    pub internetworking_model: String,

    /// If enabled, the runtime won't create a network namespace for shim and
    /// hypervisor processes.
    ///
    /// This option may have some potential impacts to your host. It should only
    /// be used when you know what you're doing.
    ///
    /// `disable_new_netns` conflicts with `internetworking_model=tcfilter` and
    /// `internetworking_model=macvtap`. It works only with
    /// `internetworking_model=none`. The tap device will be in the host
    /// network namespace and can connect to a bridge (like OVS) directly.
    ///
    /// If you are using docker, `disable_new_netns` only works with `docker run
    /// --net=none`
    #[serde(default)]
    pub disable_new_netns: bool,

    /// If specified, sandbox_bind_mounts identifies host paths to be mounted
    /// into the sandboxes shared path.
    ///
    /// This is only valid if filesystem sharing is utilized. The provided
    /// path(s) will be bind mounted into the shared fs directory. If
    /// defaults are utilized, these mounts should be available in the guest
    /// at `/run/kata-containers/shared/containers/passthrough/sandbox-mounts`.
    /// These will not be exposed to the container workloads, and are only
    /// provided for potential guest services.
    #[serde(default)]
    pub sandbox_bind_mounts: Vec<String>,

    /// If enabled, the runtime will add all the kata processes inside one
    /// dedicated cgroup.
    ///
    /// The container cgroups in the host are not created, just one single
    /// cgroup per sandbox. The runtime caller is free to restrict or
    /// collect cgroup stats of the overall Kata sandbox. The sandbox cgroup
    /// path is the parent cgroup of a container with the PodSandbox annotation.
    /// The sandbox cgroup is constrained if there is no container type
    /// annotation. See: https://pkg.go.dev/github.com/kata-containers/kata-containers/src/runtime/virtcontainers#ContainerType
    #[serde(default)]
    ...
    /// If enabled, user can run pprof tools with shim v2 process through
    /// kata-monitor.
    #[serde(default)]
    pub enable_pprof: bool,

    /// If enabled, static resource management will calculate the vcpu and
    /// memory for the sandbox/container And pod configured this will not be
    /// able to further update its CPU/Memory resource
    #[serde(default)]
    pub static_sandbox_resource_mgmt: bool,

    /// Determines whether container seccomp profiles are passed to the virtual
    /// machine and applied by the kata agent. If set to true, seccomp is
    /// not applied within the guest.
    #[serde(default)]
    ...
    ///
    /// Options:
    /// - vfio: Matches behaviour of OCI runtimes (e.g. runc) as much as
    ///   possible.  VFIO devices will appear in the container as VFIO character
    ///   devices under /dev/vfio. The exact names may differ from the host
    ///   (they need to match the VM's IOMMU group numbers rather than the
    ///   host's)
    /// - guest-kernel: This is a Kata-specific behaviour that's useful in
    ///   certain cases. The VFIO device is managed by whatever driver in the VM
    ///   kernel claims it. This means it will appear as one or more device
    ///   nodes or network interfaces depending on the nature of the device.
    ///   Using this mode requires specially built workloads that know how to
    ///   locate the relevant device interfaces within the VM.
    #[serde(default)]
    ...
    /// If keep_abnormal is enabled, it means that 1) if the runtime exits
    /// abnormally, the cleanup process will be skipped, and 2) the runtime
    /// will not exit even if the health check fails. This option is
    /// typically used to retain abnormal information for debugging.
    #[serde(default)]
    ...
    /// If enabled, the runtime will attempt to use fd passthrough feature for
    /// process io.
    #[serde(default)]
    pub use_passfd_io: bool,

    /// If fd passthrough io is enabled, the runtime will attempt to use the
    /// specified port instead of the default port.
