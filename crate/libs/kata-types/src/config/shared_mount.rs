...
pub struct SharedMount {
    ...
    /// Src_path is used to specify the path to the shared mount point in the
    /// source container. Src_path must conform to the regular expression
    /// `^(/[-\w.]+)+/?$` and cannot contain `/../`. This field cannot be
    /// omitted.
    #[serde(default)]
    pub src_path: String,
    ...
    /// Dst_path is used to specify the destination path where the shared mount
    /// point will be mounted. Dst_path must conform to the regular
    /// expression `^(/[-\w.]+)+/?$` and cannot contain `/../`. This field
    /// cannot be omitted.
    #[serde(default)]
    pub dst_path: String,
}
...
impl SharedMount {
    pub fn validate(&self) -> Result<()> {
        ...
        if !re.is_match(&self.src_path) {
            return Err(eother!(
                "shared_mount '{}': src_path is invalid. It must be an absolute path and can only \
                 contain letters, numbers, hyphens(-), underscores(_) and dots(.).",
                self.name
            ));
        }
        ...
        if !re.is_match(&self.dst_path) {
            return Err(eother!(
                "shared_mount '{}': dst_path is invalid. It must be an absolute path and can only \
                 contain letters, numbers, hyphens(-), underscores(_) and dots(.).",
                self.name
            ));
        }
    ...
    fn test_validate() {
        ...
        let tests = &[
            ...
            TestData {
                ...
                result: false,
                message: "shared_mount 'test': src_path is invalid. It must be an absolute path \
                          and can only contain letters, numbers, hyphens(-), underscores(_) and \
                          dots(.).",
            },
            TestData {
                shared_mount_annotation: r#"
                ...
                result: false,
                message: "shared_mount 'test': dst_path is invalid. It must be an absolute path \
                          and can only contain letters, numbers, hyphens(-), underscores(_) and \
                          dots(.).",
            },
            TestData {
