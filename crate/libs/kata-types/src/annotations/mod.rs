...
use crate::{
    config::{
        TomlConfig,
        hypervisor::{HugePageType, get_hypervisor_plugin},
    },
...
/// KernelModules is the annotation key for passing the list of kernel modules
/// and their parameters that will be loaded in the guest kernel.
///
/// Semicolon separated list of kernel modules and their parameters. These
/// modules will be loaded in the guest kernel using modprobe(8).
/// The following example can be used to load two kernel modules with parameters
///
///   annotations:
///     io.katacontainers.config.agent.kernel_modules: "e1000e
/// InterruptThrottleRate=3000,3000,3000 EEE=1; i915 enable_ppgtt=0"
///
/// The first word is considered as the module name and the rest as its
/// parameters.
...
/// A sandbox annotation for passing a per container path pointing at the
/// hypervisor that will run the container VM.
pub const KATA_ANNO_CFG_HYPERVISOR_PATH: &str = "io.katacontainers.config.hypervisor.path";
/// A sandbox annotation for passing a container hypervisor binary SHA-512 hash
/// value.
pub const KATA_ANNO_CFG_HYPERVISOR_HASH: &str = "io.katacontainers.config.hypervisor.path_hash";
/// A sandbox annotation for passing a per container path pointing at the jailer
/// that will constrain the container VM.
...
/// Specify the driver to be used for block device either VirtioSCSI or
/// VirtioBlock
...
/// A sandbox annotation that specifies cache-related options will be set to
/// block devices or not.
...
/// A sandbox annotation that specifies the memory space used for nvdimm device
/// by the hypervisor.
pub const KATA_ANNO_CFG_HYPERVISOR_MEMORY_OFFSET: &str =
    "io.katacontainers.config.hypervisor.memory_offset";
/// A sandbox annotation to specify if vhost-user-blk/scsi is abailable on the
/// host
pub const KATA_ANNO_CFG_HYPERVISOR_ENABLE_VHOSTUSER_STORE: &str =
    "io.katacontainers.config.hypervisor.enable_vhost_user_store";
/// A sandbox annotation to specify the directory path where vhost-user devices
...
/// A sandbox annotation for passing a per container path pointing at the kernel
/// needed to boot the container VM.
pub const KATA_ANNO_CFG_HYPERVISOR_KERNEL_PATH: &str = "io.katacontainers.config.hypervisor.kernel";
/// A sandbox annotation for passing a container kernel image SHA-512 hash
/// value.
pub const KATA_ANNO_CFG_HYPERVISOR_KERNEL_HASH: &str =
    "io.katacontainers.config.hypervisor.kernel_hash";
/// A sandbox annotation for passing a per container path pointing at the guest
/// image that will run in the container VM.
...
/// A sandbox annotation for passing a per container path pointing at the initrd
/// that will run in the container VM.
pub const KATA_ANNO_CFG_HYPERVISOR_INITRD_PATH: &str = "io.katacontainers.config.hypervisor.initrd";
/// A sandbox annotation for passing a container guest initrd SHA-512 hash
/// value.
pub const KATA_ANNO_CFG_HYPERVISOR_INITRD_HASH: &str =
    "io.katacontainers.config.hypervisor.initrd_hash";
/// A sandbox annotation for passing a per container path pointing at the guest
/// firmware that will run the container VM.
pub const KATA_ANNO_CFG_HYPERVISOR_FIRMWARE_PATH: &str =
    "io.katacontainers.config.hypervisor.firmware";
/// A sandbox annotation for passing a container guest firmware SHA-512 hash
/// value.
...
/// A sandbox annotation for passing the default vCPUs assigned for a VM by the
/// hypervisor.
pub const KATA_ANNO_CFG_HYPERVISOR_DEFAULT_VCPUS: &str =
    "io.katacontainers.config.hypervisor.default_vcpus";
/// A sandbox annotation that specifies the maximum number of vCPUs allocated
/// for the VM by the hypervisor.
...
/// A sandbox annotation used to indicate if devices need to be hotplugged on
/// the root bus instead of a bridge.
...
/// A sandbox annotation to specify the type of machine being emulated by the
/// hypervisor.
pub const KATA_ANNO_CFG_HYPERVISOR_MACHINE_TYPE: &str =
    "io.katacontainers.config.hypervisor.machine_type";
/// A sandbox annotation to specify machine specific accelerators for the
/// hypervisor.
pub const KATA_ANNO_CFG_HYPERVISOR_MACHINE_ACCELERATORS: &str =
    "io.katacontainers.config.hypervisor.machine_accelerators";
/// EntropySource is a sandbox annotation to specify the path to a host source
/// of entropy (/dev/random, /dev/urandom or real hardware RNG device)
...
/// A sandbox annotation to specify the memory slots assigned to the VM by the
/// hypervisor.
...
/// A sandbox annotation to specify if the memory should be pre-allocated from
/// huge pages.
...
/// A sandbox annotation that specifies max rate on network I/O inbound
/// bandwidth.
...
/// A sandbox annotation that specifies max rate on network I/O outbound
/// bandwidth.
...
/// A sandbox annotation to specify the path within the VM that will be used for
/// 'drop-in' hooks.
pub const KATA_ANNO_CFG_HYPERVISOR_GUEST_HOOK_PATH: &str =
    "io.katacontainers.config.hypervisor.guest_hook_path";
/// A sandbox annotation to enable rootless hypervisor (only supported in QEMU
/// currently).
...
/// A sandbox annotation to specify the shared file system type, either
/// inline-virtio-fs (default), virtio-9p, virtio-fs or virtio-fs-nydus.
...
/// GPU specific annotations for remote hypervisor to help with instance
/// selection It's for minimum number of GPUs required for the VM.
...
/// A sandbox annotation that determines if seccomp should be applied inside
/// guest.
...
/// A sandbox annotaion that determines how the VM should be connected to the
/// the container network interface.
pub const KATA_ANNO_CFG_INTER_NETWORK_MODEL: &str =
    "io.katacontainers.config.runtime.internetworking_model";
/// SandboxCgroupOnly is a sandbox annotation that determines if kata processes
/// are managed only in sandbox cgroup.
pub const KATA_ANNO_CFG_SANDBOX_CGROUP_ONLY: &str =
    "io.katacontainers.config.runtime.sandbox_cgroup_only";
/// A sandbox annotation that determines if create a netns for hypervisor
/// process.
...
/// An annotation to declare shared mount points, which is a set of mount points
/// that directly share mounted objects between containers.
...
/// A sandbox annotation used to specify prefetch_files.list host path container
/// image being used,
/// and runtime will pass it to Hypervisor to  search for corresponding prefetch
/// list file. "io.katacontainers.config.hypervisor.prefetch_files.list"
///                                =
                        ...
                        Ok(num_cpus) => {
                                    ...
                                    .get_max_cpus() as f32
                            {
                                return Err(io::Error::new(
                                    io::ErrorKind::InvalidData,
                                    format!(
                                        "Vcpus specified in annotation {} is more than maximum \
                                         limitation {}",
                                        num_cpus,
                                        get_hypervisor_plugin(hypervisor_name)
                                            .unwrap()
                            ...
                            Ok(mem_bytes) => {
                                        ...
                                        .get_min_memory()
                                {
                                    return Err(io::Error::new(
                                        io::ErrorKind::InvalidData,
                                        format!(
                                            "memory specified in annotation {} is less than \
                                             minimum limitation {}",
                                            memory_size,
                                            get_hypervisor_plugin(hypervisor_name)
                                                .unwrap()
