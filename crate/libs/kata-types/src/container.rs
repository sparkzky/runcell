...
impl State {
    /// Check whether it's a valid state transition from self to the
    /// `new_state`.
...
///
/// This function creates a new `HashMap` containing the updated annotations,
/// ensuring that the original map remains unchanged. It is optimized for
/// performance by pre-allocating the necessary capacity and handling removals
/// and additions efficiently.
///
/// **Note:** If a key from `keys_to_add` already exists in the new map (either
/// from the original annotations or from another entry in `keys_to_add`), the
/// corresponding add operation will be **ignored**. The existing value will not
/// be overwritten.
///
/// # Arguments
///
/// * `annotations` - A reference to the original `HashMap` of annotations.
/// * `keys_to_remove` - A slice of string slices (`&[&str]`) representing the
///   keys to be removed.
/// * `keys_to_add` - A slice of tuples (`&[(String, String)]`) representing the
...
mod tests {
    ...
    fn test_update_annotations_not_in_annotations_map() {
        ...
        // a `keys_to_remove` that isn't currently in the annotations map is passed in
        // with "key4"
        let updated = update_ocispec_annotations(&annotations, &keys_to_remove, &keys_to_add);
